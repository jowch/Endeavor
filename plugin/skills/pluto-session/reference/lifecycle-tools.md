...
| `open_notebook` | Load an existing `.jl` file into the session | Requires a user-specified path; returns `notebook_in_use` if that path is already open; defaults to safe preview |
| `new_notebook` | Create a new, empty notebook — written by Pluto itself — and load it | Optional `path`; never overwrites an existing file; defaults to safe preview |
